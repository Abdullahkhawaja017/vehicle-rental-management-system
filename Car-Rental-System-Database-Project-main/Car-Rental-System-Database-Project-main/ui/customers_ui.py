...
import controllers.customer_controller as cust_c
...
def show_customers_window(parent=None):
    ...
    window = tk.Toplevel()
    window.title("Customer Management - Car Rental System")
    ...
    # Sidebar Info
    tk.Label(left_panel, text="Customer Database", font=("Segoe UI", 20, "bold"), 
             bg=COLOR_SIDEBAR, fg="white").pack(padx=20, anchor="w")
    
    tk.Label(left_panel, text="View and manage registered\ncustomer profiles.", font=("Segoe UI", 10), 
    ...
    tk.Label(header_frame, text="Registered Customers", font=FONT_HEADER, 
    ...
    # Columns
    columns = ("ID", "Name", "Email", "Phone", "License", "Username")
    ...
    tree.heading("License", text="Driver License")
    ...
    def refresh_data():
        for item in tree.get_children():
            tree.delete(item)
        try:
            customers = cust_c.get_all_customers()
            for cust in customers:
                # cust tuple: (id, name, email, phone, address, license, username, pass, role, created)
                # Map indices: 0=id, 1=name, 2=email, 3=phone, 5=license, 6=username
                tree.insert("", tk.END, values=(cust[0], cust[1], cust[2], cust[3], cust[5], cust[6]))
        except Exception as e:
            messagebox.showerror("Error", f"Could not load customers: {e}")

    refresh_data()

    # Footer Status
    tk.Label(container, text="* List shows all active customer accounts.", 
             font=("Segoe UI", 9, "italic"), bg=COLOR_BG_MAIN, fg=COLOR_TEXT_LIGHT).pack(anchor="w", pady=(10, 0))