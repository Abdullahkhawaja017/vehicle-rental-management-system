...
def show_my_bookings_window(parent=None):
    ...
    window = tk.Toplevel()
    window.title("My Bookings - Car Rental System")
    ...
    # Sidebar Info
    tk.Label(left_panel, text="My Rentals", font=("Segoe UI", 20, "bold"), 
             bg=COLOR_SIDEBAR, fg="white").pack(padx=20, anchor="w")
    
    tk.Label(left_panel, text="Track your current and\npast vehicle rentals.", font=("Segoe UI", 10), 
    ...
    tk.Label(header_frame, text="Booking Status", font=FONT_HEADER, 
             bg=COLOR_BG_MAIN, fg=COLOR_TEXT_DARK).pack(side="left")

    # Refresh Button (Top Right)
    def trigger_refresh():
        refresh_data()
        messagebox.showinfo("Refreshed", "Data updated successfully.", parent=window)
    ...
    def refresh_data():
        ...
        try:
            bookings = bc.get_my_bookings(user['user_id'])
            for b in bookings:
                # b: (id, make, model, start, end, status, return_date)
                car_name = f"{b[1]} {b[2]}"
                tree.insert("", tk.END, values=(b[0], car_name, b[3], b[4], b[5]))
    ...
    def handle_action():
        ...
        if not selected:
            messagebox.showwarning("Warning", "Please select a booking.", parent=window)
            return
        ...
        if status == "Active":
            ...
            confirm = messagebox.askyesno("Return Car", "Request to return this car?", parent=window)
            if confirm:
                success, msg = bc.request_return_booking(booking_id)
                if success:
        ...
        elif status == "ReturnRequested":
            messagebox.showinfo("Info", "You have already requested a return.\nPlease wait for an employee to accept it.", parent=window)
            
        elif status == "Completed":
            # Action: Show Bill
            confirm = messagebox.askyesno("View Bill", "This rental is complete. View your bill history?", parent=window)
            if confirm:
                show_my_bills_window()
            
        else:
            messagebox.showinfo("Info", f"Status: {status}", parent=window)

    # Styled Action Button
    btn_action = tk.Button(action_bar, text="⚡ Take Action (Return / View Bill)", font=FONT_BTN, 
    ...
    tk.Label(action_bar, text="Select a booking to view available actions.", 
